# 📋 TaskFlow – Task Management System

## 📌 Project Overview
TaskFlow is a web-based task management application developed using Agile Scrum methodology over two sprints. It demonstrates iterative development, collaborative coding via GitHub, and Scrum practices.

## 👥 Team Members & Roles
| Name | Role | Responsibilities | GitHub Username |
|------|------|------------------|-----------------|
| [Member 1 Name] | Product Owner | Product vision, user stories, documentation | @username1 |
| [Member 2 Name] | Scrum Master | Sprint planning, Scrum tracking, GitHub board | @username2 |
| [Member 3 Name] | Developer | System development, testing, implementation | @username3 |

## 🎯 Product Vision & Scope
[Copy from your earlier draft]

## 📋 Scrum Process Documentation
Our development followed Scrum methodology with 2 sprints:

### Sprint 1 (Week 1): Core Functionality
- **Goal**: Develop basic task creation and viewing
- **Completed**: Repository setup, UI layout, PB1, PB2
- **Screenshot**: ![Sprint 1 Board](scrum-artifacts/screenshots/sprint1-board.png)

### Sprint 2 (Week 2): Enhanced Features
- **Goal**: Complete all remaining functionality
- **Completed**: PB3, PB4, PB5, PB6, testing, documentation
- **Screenshot**: ![Sprint 2 Board](scrum-artifacts/screenshots/sprint2-board.png)

### GitHub Collaboration Evidence
- **Commit History**: ![Commit Graph](scrum-artifacts/screenshots/commit-graph.png)
- **Pull Requests**: ![PRs](scrum-artifacts/screenshots/pull-requests.png)
- **Project Board**: ![Project Board](scrum-artifacts/screenshots/project-board-full.png)

## ⚙️ Technologies Used
- HTML5, CSS3, JavaScript (ES6+)
- Browser Local Storage API
- GitHub (Version Control & Project Management)
- Agile Scrum Methodology

## 🚀 Getting Started

### Prerequisites
- Modern web browser (Chrome 90+, Firefox 88+, Safari 14+)
- Git (for development)

### Installation
1. Clone the repository:
   ```bash
   git clone https://github.com/[username]/TaskFlow-Task-Management.git
